...
func TestMFAEnable_Success(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal(setupRec.Body.Bytes(), &setupResp))
	...
	enableReq := httptest.NewRequest(http.MethodPost, "/api/user/mfa/enable", bytes.NewReader(enableJSON))
	...
	assert.Equal(t, true, enableResp["success"])
...
func TestMFAEnable_WrongCode(t *testing.T) {
	...
	assert.Equal(t, http.StatusUnauthorized, enableRec.Code)
}

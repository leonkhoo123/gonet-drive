...
func ShareFileRoutes(router *gin.Engine, shareRepo repository.SharingRepository, jwtSvc *gonetjwt.Service) {
	// Share File Access (requires shareJwt token)
	fileAccess := router.Group("/api/share/file")
	fileAccess.Use(middleware.ShareAuthMiddleware(config.AppConfig, shareRepo, jwtSvc))
	{
...
func ensureWithinAuthorizedPath(authorizedPath string, requestedSubPath string) (string, bool) {
	...
	// Strip leading slash so filepath.Join treats it as relative, not absolute
	requestedSubPath = strings.TrimPrefix(requestedSubPath, "/")
...
func ShareFileList(c *gin.Context) {
	...
	if !safe {
		httpx.Err(c, http.StatusForbidden, "Path traversal detected or unauthorized path")
		return
	}
...
func ShareFileProperties(c *gin.Context) {
	...
	if err != nil {
		logger.L.Error("share file properties failed", "err", err)
...
func ShareFileDelete(c *gin.Context) {
	...
	for _, p := range req.Sources {
		targetPath, safe := ensureWithinAuthorizedPath(authorizedPath.(string), p)
		if !safe {
			httpx.Err(c, http.StatusForbidden, "Path traversal detected or unauthorized path: "+p)
			return
		}
	...
	err := service.ShareDeletePermanentFiles(req, config.AppConfig, c.GetString("request_id"), "share:"+c.GetString("share_id"))
	if err != nil {
		httpx.Err(c, http.StatusInternalServerError, "Failed to permanently delete files")
		return
	}

	httpx.Msg(c, http.StatusOK, "Files permanently deleted successfully")
}
...
func ShareCreateFolder(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		httpx.Err(c, http.StatusBadRequest, "Invalid request format")
		return
	}
	...
	if err != nil {
		logger.L.Error("share create folder failed", "err", err, "dir", req.Dir, "folderName", req.FolderName)
		httpx.Err(c, http.StatusInternalServerError, "failed to create folder")
		return
	}

	httpx.Msg(c, http.StatusOK, "Folder created successfully")
}
...
func ShareFileServe(c *gin.Context) {
	...
	if !safe {
		httpx.Abort(c, http.StatusForbidden, "Path traversal detected or unauthorized path")
		return
	}
...
func ShareFileDeleteSoft(c *gin.Context) {
	...
	err := service.ShareDeleteFiles(req, config.AppConfig, c.GetString("request_id"), "share:"+c.GetString("share_id"))
	if err != nil {
		httpx.Err(c, http.StatusInternalServerError, "Failed to delete files")
		return
	}

	httpx.Msg(c, http.StatusOK, "Files deleted successfully")
}
...
func ShareFileRename(c *gin.Context) {
	...
	if err := service.ShareRenameFile(req, config.AppConfig); err != nil {
		logger.L.Error("share rename file failed", "err", err, "source", req.Source, "newName", req.NewName)
		httpx.Err(c, http.StatusInternalServerError, "failed to rename file")
		return
	}

	httpx.Msg(c, http.StatusOK, "File renamed successfully")
}
...
func ShareFileCopy(c *gin.Context) {
	...
	if err := service.ShareCopyFiles(req, config.AppConfig, c.GetString("request_id"), "share:"+c.GetString("share_id")); err != nil {
		logger.L.Error("share copy files failed", "err", err)
		httpx.Err(c, http.StatusBadRequest, "failed to copy files")
		return
	}

	httpx.Msg(c, http.StatusOK, "Files copied successfully")
}
...
func ShareFileMove(c *gin.Context) {
	...
	if err := service.ShareMoveFiles(req, config.AppConfig, c.GetString("request_id"), "share:"+c.GetString("share_id")); err != nil {
		logger.L.Error("share move files failed", "err", err)
		httpx.Err(c, http.StatusBadRequest, "failed to move files")
		return
	}

	httpx.Msg(c, http.StatusOK, "Files moved successfully")
}

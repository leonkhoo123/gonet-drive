...
func copyFilesHandler(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := service.CopyFiles(req, cfg, requestID, username); err != nil {
			logger.L.Error("copy files failed", "err", err, "username", username)
			httpx.Err(c, http.StatusBadRequest, "failed to start copy operation")
			return
		}
		httpx.Msg(c, http.StatusOK, "Copy operation started")
	}
}
...
func moveFilesHandler(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		requestID := c.GetString("request_id")
		username := c.GetString(authgin.KeyUsername)
		if err := service.MoveFiles(req, cfg, requestID, username); err != nil {
			logger.L.Error("move files failed", "err", err, "username", username)
			httpx.Err(c, http.StatusBadRequest, "failed to start move operation")
			return
		}
		httpx.Msg(c, http.StatusOK, "Move operation started")
	}
}
...
func cancelOperationHandler(c *gin.Context) {
	...
	username := c.GetString(authgin.KeyUsername)
	if err := service.CancelOperation(req, username); err != nil {
		logger.L.Warn("cancel operation denied", "err", err, "username", username, "opId", req.OpID)
		httpx.Err(c, http.StatusForbidden, "failed to cancel operation")
		return
	}
	httpx.Msg(c, http.StatusOK, "Operation cancelled")
}
...
func deleteFilesHandler(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		requestID := c.GetString("request_id")
		username := c.GetString(authgin.KeyUsername)
		if err := service.DeleteFiles(req, cfg, requestID, username); err != nil {
			logger.L.Error("delete files failed", "err", err, "username", username)
			httpx.Err(c, http.StatusBadRequest, "failed to start delete operation")
			return
		}
		httpx.Msg(c, http.StatusOK, "Delete operation started")
	}
}
...
func deletePermanentFilesHandler(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		requestID := c.GetString("request_id")
		username := c.GetString(authgin.KeyUsername)
		if err := service.DeletePermanentFiles(req, cfg, requestID, username); err != nil {
			logger.L.Error("permanent delete files failed", "err", err, "username", username)
			httpx.Err(c, http.StatusBadRequest, "failed to start permanent delete operation")
			return
		}
		httpx.Msg(c, http.StatusOK, "Permanent delete operation started")
	}
}
...
func renameFileHandler(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := service.RenameFile(req, cfg); err != nil {
			logger.L.Error("rename file failed", "err", err, "source", req.Source, "newName", req.NewName)
...
func createFolderHandler(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := service.CreateFolder(req, cfg); err != nil {
			logger.L.Error("create folder failed", "err", err, "dir", req.Dir, "folderName", req.FolderName)
...
func filePropertiesHandler(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			logger.L.Error("get file properties failed", "err", err)

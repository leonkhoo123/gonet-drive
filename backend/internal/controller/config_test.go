...
func TestListConfigs_Valid(t *testing.T) {
	router, _, _, _, _, db := setupConfigRouter(t)
	testutil.CreateTestUser(t, db, "configadmin", "pass123", "admin")

	accessCookie := testutil.LoginAndGetCookie(t, router, "configadmin", "pass123")

	rec := testutil.MakeAuthRequest(t, router, http.MethodGet, "/api/user/admin/config", nil, accessCookie)
...
func TestUpdateConfig_Valid(t *testing.T) {
	router, _, _, _, _, db := setupConfigRouter(t)
	testutil.CreateTestUser(t, db, "configadmin2", "pass123", "admin")

	accessCookie := testutil.LoginAndGetCookie(t, router, "configadmin2", "pass123")

	// First list configs to get an ID
	listRec := testutil.MakeAuthRequest(t, router, http.MethodGet, "/api/user/admin/config", nil, accessCookie)
	require.Equal(t, http.StatusOK, listRec.Code)

	var listEnvelope map[string]interface{}
	require.NoError(t, json.Unmarshal(listRec.Body.Bytes(), &listEnvelope))
	listData := listEnvelope["data"].(map[string]interface{})
	configs := listData["configs"].([]interface{})
	require.NotEmpty(t, configs)

	firstID := int(configs[0].(map[string]interface{})["id"].(float64))
	...
	req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/api/user/admin/config/%d", firstID), bytes.NewReader(jsonBody))
...
func TestUpdateConfig_InvalidID(t *testing.T) {
	router, _, _, _, _, db := setupConfigRouter(t)
	testutil.CreateTestUser(t, db, "configadmin3", "pass123", "admin")

	accessCookie := testutil.LoginAndGetCookie(t, router, "configadmin3", "pass123")
	...
	req := httptest.NewRequest(http.MethodPut, "/api/user/admin/config/abc", bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(accessCookie)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
...
func TestUpdateConfig_NoAuth(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPut, "/api/user/admin/config/1", bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
...
func TestUpdateConfig_NotFound(t *testing.T) {
	router, _, _, _, _, db := setupConfigRouter(t)
	testutil.CreateTestUser(t, db, "configadmin4", "pass123", "admin")

	accessCookie := testutil.LoginAndGetCookie(t, router, "configadmin4", "pass123")
	...
	req := httptest.NewRequest(http.MethodPut, "/api/user/admin/config/99999", bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(accessCookie)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)

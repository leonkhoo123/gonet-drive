...
func (s *AudiobookService) GetAudioBookPaths(c *gin.Context) {
	...
	httpx.OK(c, http.StatusOK, gin.H{"paths": paths})
}
...
func (s *AudiobookService) AddAudioBookPath(c *gin.Context) {
	...
	if err != nil {
		httpx.Err(c, http.StatusInternalServerError, "Failed to add path or duplicate path")
		return
	}

	httpx.Msg(c, http.StatusOK, "Path added successfully")
}
...
func (s *AudiobookService) UpdateAudioBookPath(c *gin.Context) {
	...
	if err != nil {
		httpx.Err(c, http.StatusBadRequest, "Invalid ID")
		return
	}
	...
	if req.IsEnabled != nil {
		...
		if err != nil {
			httpx.Err(c, http.StatusInternalServerError, "Failed to update path")
			return
		}
	}

	httpx.Msg(c, http.StatusOK, "Path updated successfully")
}
...
func (s *AudiobookService) DeleteAudioBookPath(c *gin.Context) {
	...
	if err != nil {
		httpx.Err(c, http.StatusInternalServerError, "Failed to delete path")
		return
	}

	httpx.Msg(c, http.StatusOK, "Path deleted successfully")
}
...
func (s *AudiobookService) ReportAudioBookProgress(c *gin.Context) {
	...
	if err != nil {
		httpx.Err(c, http.StatusInternalServerError, "Failed to update progress")
		return
	}

	httpx.Msg(c, http.StatusOK, "Progress updated successfully")
}
...
func (s *AudiobookService) ListAudioBooks(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		httpx.OK(c, http.StatusOK, gin.H{"audiobooks": audioBooks})
	}
}
...
func (s *AudiobookService) StreamAudioBook(cfg *config.CloudConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !util.IsSafePathComponent(name) {
			httpx.Err(c, http.StatusBadRequest, "Invalid name")
			return
		}
		...
		if foundPath == "" {
			httpx.Err(c, http.StatusNotFound, "Audiobook not found")
			return
		}

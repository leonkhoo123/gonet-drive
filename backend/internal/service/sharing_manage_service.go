...
func (s *SharingService) CreateShareEndpoint(c *gin.Context) {
	...
	if err != nil {
		logger.L.Error("failed to generate random PIN", "err", err)
		httpx.Err(c, http.StatusInternalServerError, "Failed to generate PIN")
		return
	}

	hash, err := bcrypt.GenerateFromPassword([]byte(pin), BcryptCost)
	if err != nil {
		httpx.Err(c, http.StatusInternalServerError, "Failed to hash PIN")
		return
	}
...
func (s *SharingService) ListSharesEndpoint(c *gin.Context) {
	...
	httpx.OK(c, http.StatusOK, gin.H{"shares": shares})
}
...
func (s *SharingService) ToggleShareBlockedEndpoint(c *gin.Context) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			httpx.Err(c, http.StatusNotFound, "Share link not found")
			return
		}
		httpx.Err(c, http.StatusInternalServerError, "Database error")
		return
	...
	if err := s.ShareRepo.UpdateBlockedStatus(id, username.(string), newBlocked); err != nil {
		httpx.Err(c, http.StatusInternalServerError, "Failed to update blocked status")
		return
	}
	...
	httpx.OK(c, http.StatusOK, gin.H{"message": "Status updated", "blocked": newBlocked})
}
...
func (s *SharingService) DeleteShareEndpoint(c *gin.Context) {
	...
	if rowsAffected == 0 {
		httpx.Err(c, http.StatusNotFound, "Share link not found or already deleted")
		return
	}
	...
	httpx.Msg(c, http.StatusOK, "Share link deleted")
}

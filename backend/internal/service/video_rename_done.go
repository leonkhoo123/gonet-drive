...
func VideoRenameDone(c *gin.Context, cfg *config.CloudConfig) {
	...
	if err != nil {
		httpx.Err(c, http.StatusBadRequest, "Invalid filename: "+err.Error())
		return
	}
	...
	if err := os.MkdirAll(doneDir, 0777); err != nil {
		httpx.Err(c, http.StatusInternalServerError, "failed to create done folder")
		return
	}
	...
	if req.RotateAngle != 0 {
		...
		if err != nil {
			httpx.Err(c, http.StatusInternalServerError, err.Error())
			return
		}
	...
	if err := os.Rename(srcPath, destPath); err != nil {
		httpx.Err(c, http.StatusInternalServerError, fmt.Sprintf("failed to move file: %v", err))
		return
	}
